...
export default function ProductUploadPage({ kioskId }: ProductUploadPageProps) {
  ...
  const [kioskCap, setKioskCap] = useState<string>("");
  ...
  useEffect(() => {
    const fetchCaps = async () => {
      const data = await getSingleKioskCap(
        currentAccount?.address || "",
        kioskId
      );

      setKioskCap(data?.objectId || "");
    };
    fetchCaps();
  }, [currentAccount]);
  ...
  useEffect(() => {
    try {
      const storedData = sessionStorage.getItem(
        "@enoki/flow/state/enoki_public_e5a1d53741cdbe61403b4c6de297ca10/testnet"
      );
      if (storedData) {
  ...
  const handleListItem = async ({
    ...
    price,
    quantity,
  }: HandleListItemParams): Promise<void> => {
    setLoading(true);

    try {
      const tx = new TransactionBlock();
      tx.moveCall({
        target: `${process.env.NEXT_PUBLIC_PACKAGE_ID}::marketplace::create_product_type`,
        arguments: [
